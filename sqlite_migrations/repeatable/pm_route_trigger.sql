...
BEGIN
    INSERT INTO pm_route_hierarchy (route_id, ancestor_route_id, depth)
    SELECT NEW.route_id, ancestor_route_id, depth
    FROM pm_route_hierarchy
    ...
    SET path = (
        ...
        FROM (
            ...
            FROM pm_route_hierarchy JOIN pm_route AS ancestor ON ancestor.route_id = pm_route_hierarchy.ancestor_route_id
            WHERE pm_route_hierarchy.route_id = pm_route.route_id
            ORDER BY pm_route_hierarchy.depth
        ) AS ancestors
    )
...
BEGIN
    DELETE FROM pm_route_hierarchy WHERE route_id = OLD.route_id;
END;
...
BEGIN
    ...
    WHERE pm_route.route_id = NEW.route_id OR EXISTS (
        SELECT 1 FROM pm_route_hierarchy WHERE ancestor_route_id = NEW.route_id
    );
END;
